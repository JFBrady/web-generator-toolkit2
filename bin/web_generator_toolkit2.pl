...
web_generator_toolkit2 -options
...
=head1 OPTIONS
 ...
 -t copy input files to tstamp dir(3) + column YYYYMMDDhhmmss(1) or hh:mm:ss(2)
 -y set arr, rt, byte probability levels (default is 90_95_99)

=head1 EXAMPLES

 web_generator_toolkit2.pl
 web_generator_toolkit2.pl -d -g -t 1
 web_generator_toolkit2.pl -d -g -t 1 -y 80_85_90

=cut

##########################################################################
# Perl Script:                                                           #
# Purpose: uses jmeter csv output to produce arr,rt,byte,concur stats    #
# Author:  James F Brady                                                 #
# Release: Version 1.0.0                                                 #
# Copyright © 2020 James F. Brady                                        #
##########################################################################
require 5.004;
use strict;
...
my ($interarrival_ms_count);
my ($response_ms_ref);
my ($response_ms_stats);
my ($response_ms_count);
...
my ($concurrency_ref);
my (@concurrency_ms_list);
my ($concurrency_ms_stats);
my ($concurrentEvents_ref);
my ($pstate_ref);
my ($pstate_count);
my ($pstate_val);
my ($state_probs_ref);
...
my (%graph_type)=('agg'=>'Event Statistics',
                  ...
                  'byte'=>'Web Event Size (bytes)',
                  'concur'=>'Concurrent Events (Count)',
...
my ($hist_head)=",------------Statistics------------";
my ($col_head)="TimeStamp_ms,REvent_ms,WebEventName,ResponseCode," .
...
foreach $infile (@$infile_list_ref)
{
  ...
  while (<INFILE>)
  {
    ...
    if (@row_parts >= 6)
    {
      ...
      $web_event_label_orig = $row_parts[2];
    ...
    ###################################################
    # Convert web_event_label special characters to _ #
    ###################################################
    ($web_event_label) = convert_web_event_label($web_event_label_orig);
    ...
    ################################
    push @{$time_stamps{$web_event_label}},$rtstamp_row;
  ...
  # For each web_event           #
  ################################
  foreach $web_event_label (sort keys %time_stamps)
  {
    foreach $tstamp (sort @{$time_stamps{$web_event_label}})
    {
    ...
    #########################################
    if ($web_event_label eq '~Total')
    {
      $web_event = 'Total';
    }
    else
    {
      $web_event = $web_event_label;
    }

    ########################
    # Print web_event info #
    ########################
    print "$web_event\n";
    ...
    ##################################
    ($concurrentEvents_ref,$pstate_ref) = concurrency($concurrency_ref);
    ...
    {
      $interarrival_ms_stats = create_statistics($web_event,
                                                 $datatime_ms,
                                                 'cv',
                                                 $prob1,
                                                 $prob2,
    ...
    if (@$response_ms_ref)
    {
      $response_ms_stats = create_statistics($web_event,
                                             $datatime_ms,
                                             $cv_or_vmr,
                                             $prob1,
                                             $prob2,
    ...
    if (@$web_bytes_ref)
    {
      $web_bytes_stats = create_statistics($web_event,
                                           $datatime_ms,
                                           $cv_or_vmr,
                                           $prob1,
                                           $prob2,
    ...
    {
      $response_1st_ms_stats = create_statistics($web_event,
                                                 $datatime_ms,
                                                 $cv_or_vmr,
                                                 $prob1,
                                                 $prob2,
    ...
    #################################################
    # Create Histograms For concur, arr, rt - opt_d #
    #################################################
    if ($opt_d)
    {
      #############################################
      # Concurrent Event Statistics               #
      #############################################
      if (@$pstate_ref)
      {
        $pstate_count = @$pstate_ref;
        ##########################################
        # Histograms - concur                    #
        ##########################################
        ($state_probs_ref) = histogram_horizontal('State',
                                                  $pstate_count,
                                                  $pstate_ref);
        ###########
        # Heading #
        ###########
        unshift @$state_probs_ref,$hist_head;
        unshift @$state_probs_ref,
        "Concurrent Events (Count) {$web_event} - $dir_infile1_daydate";
      ...
      if (@$interarrival_ms_ref)
      {
        $interarrival_ms_count = @$interarrival_ms_ref;
        ##########################################
        # Histograms - arr                       #
        ##########################################
        ($arr_1_ref) = create_histogram($cell_size1,$cell_max,
                                        $interarrival_ms_ref);
        ($arr_1h_ref) = histogram_horizontal('MilliSec',
                                             $interarrival_ms_count,
                                             $arr_1_ref);
        ###########
        # Heading #
        ###########
        unshift @$arr_1h_ref,$hist_head;
        unshift @$arr_1h_ref,
        "Inter-arrival Time Histogram For {$web_event} - $dir_infile1_daydate";
      ...
      if (@$response_ms_ref)
      {
        $response_ms_count = @$response_ms_ref;
        ##########################################
        # Histograms - rt                        #
        ##########################################
        ($rt_1_ref) = create_histogram($cell_size2,$cell_max,
                                       $response_ms_ref);
        ($rt_1h_ref) = histogram_horizontal('MilliSec',
                                            $response_ms_count,
                                            $rt_1_ref);
        ###########
        # Heading #
        ###########
        unshift @$rt_1h_ref,$hist_head;
        unshift @$rt_1h_ref,
        "Response Time Histogram For {$web_event} - $dir_infile1_daydate";
  ...
  if (@web_bytes_list)
  {
    ...
    "Web Event Size Summary Statistics (bytes) - $dir_infile1_daydate";
    ###########
    # Outfile #
    ###########
  ...
  #####################################
  # create concurrency csv file       #
  #####################################
  if (@concurrency_ms_list)
  {
    ###########
    # Heading #
    ###########
    unshift @concurrency_ms_list,$stat_head;
    unshift @concurrency_ms_list,
    "Concurrent Event Statistics (Count) - $dir_infile1_daydate";
    ###########
    # Outfile #
    ###########
    $outfile = join '_',$dir_infile1_yyyymmdd,'concur';
    $outfile = join '.',$outfile,'csv';
    print "  - create $outfile\n";
    create_output_file($outdir_rpt,'',$outfile,\@concurrency_ms_list);
  }
  ...
  ################################
  # Histograms concur arr and rt #
  ################################
  if ($opt_d)
  {
    print "histograms\n";
    print "  - create concur histogram files for each event and total\n";
    print "  - create arr histogram files for each event and total\n";
    print "  - create rt histogram files for each event and total\n";
  }
  ...
  {
    print "graphs\n";
    print "  - create graph files for each event and total by statistic\n";
  }
...
sub
convert_web_event_label
{
  my ($web_event_label_in) = @_;

  my (@web_event_label);
  my ($web_event_label_out);
  my ($web_event_label_next);
  my (@special_char) = (' ','/',':','\*','\?','\"','<','>','\|','#');
  my ($char);
  ...
  # Convert web_event_label special characters to _ #
  ###################################################
  $web_event_label_next = $web_event_label_in;
  foreach $char (@special_char)
  {
    @web_event_label = split ($char,$web_event_label_next);
    $web_event_label_out = join '_',@web_event_label;
    $web_event_label_next = $web_event_label_out; 
  }

  return($web_event_label_out);
}
...
create_data_lists
{
  ...
  if (@$time_stamp_ref)
  {
    foreach $tstamp (@$time_stamp_ref)
    {
      ...
      $concurrency_val = join ',',$arrival_ticks_ms,$response_ms;
      ...
      push @concurrency_list,$concurrency_val;
...
create_statistics
{
  my ($web_event,$datatime_ms_val,$cv_or_vmr1,
  ...
  $stats = sprintf('%s,%d,%.2f,%d,%.2f,%.2f,%.2f,%d,%d,%d,%d,%d',
                    $web_event,$n,$tps,$median,$mean,$sdev,$disp,
...
create_histogram
{
  ...
  if ($overflow)
  {
    $cell_overflow=$cell[$cell_count]+1;
    $hist = join (',',$cell_overflow,$overflow);
    push @histogram,$hist;
  }    
...
{
  my ($cell_label,$count,$histogram_ref) = @_;
  ...
  ##############################
  # Initialize cell_save       #
  ##############################
  $cell_save = $cell_label;

  ##############################
  # Create Freq and Prob lists #
  ##############################
  foreach $hist_val (@$histogram_ref)
  {
    if ($cell_label eq 'State')
    {
      ($cell,$freq,$prob) = split('\,',$hist_val); 
    }
    else
    {
    ...
    $ex = $ex+$cell*$prob;
    $ex2 = $ex2+$cell**2*$prob;
    $cell_save = join ',',$cell_save,$cell;
    $prob_save = join ',',$prob_save,$prob;
    $freq_save = join ',',$freq_save,$freq;
  }
  $variance=$ex2-$ex**2; 
  $sdev=sqrt($variance); 
  $stats = join ',',"Value",$ex,$sdev,$variance,$cell_max,$p_sum,$freq_sum;
  push @probs,"Statistic,mean,sdev,var,max,p_sum,freq_sum";
  push @probs,$stats;
  push @probs,$cell_save,$prob_save,$freq_save;
...
create_output_file
{
  my ($outdir,$outdir2,$outfile,$list_ref) = @_;
  ...
  {
    ###################################################
    # Create output directory(s) and open output file #
    ###################################################
    mkdir $outdir,0777;
    if ($outdir2)
    {
      $outdir = join('/',$outdir,$outdir2);
      mkdir $outdir,0777;
    }
...
{
  my ($outdir,$outdir2,$outfile,$attribute,$graph_data_ref) = @_;
